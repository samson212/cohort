...
This builds on `01-complexity.md` rather than repeating it. 01-complexity.md's findings are
...
|---|---|---|---|
| high | data model | A branch is modelled four times — `worktrees`, `orphans`, `remote_only`, `open_prs` — each with its own collector, status vocabulary, sort call and row renderer, which is the root cause of every duplication 01-complexity.md lists downstream. | Collect one flat `branches` list keyed by branch name with `path`/`on_remote`/`pr` as optional attributes, and let the page group it for display. |
...
| low | config ownership | Beyond 01-complexity.md's port finding, the refresh interval has four owners — `DEFAULT_REFRESH`, the unit's `--refresh 30`, the page's `setInterval(30000)` and the footer's literal "every 30s". | Serve the interval in the JSON payload and have the page and footer read it from there. |
...
The biggest absolute win and the one that makes 01-complexity.md's four "duplicated
...
01-complexity.md's #1 finding (three-way port ownership) is fixed for free. Risk is
...
01-complexity.md's verdict — *the three-stage shape is defensible* — is the finding I
...
ordering and rendering duplications 01-complexity.md correctly identified do not need
