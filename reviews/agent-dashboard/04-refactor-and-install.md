...
| **major** | Stale unit carries `--port 6283` in ExecStart, silently overriding the config file — the exact port-ownership bug `01-complexity.md` #1 flagged; on this VM the *old* unit still passes it. | The committed static unit correctly omits `--port` (good); apply the same to any lingering generated unit (fix above). |
...
04-refactor-and-install.md written to the worktree (uncommitted) — matches the precedent set by 2055a8c, which committed the first three review files by an explicit follow-up commit; left uncommitted here since the task said "your call" and the branch is otherwise clean.
