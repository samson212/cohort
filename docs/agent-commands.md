...
Reference for the custom slash commands in `commands/` — what each
...
Two custom commands live under `commands/`:
...
- **`/git-commit`** — the wrap-up command: looks at everything changed (staged,
  ...
  uses one given verbatim), commits via `just commit <message-file>`, then follows
  `/git-push`'s own sync procedure and pushes via `just push`. Absorbed what used to
...
- **`/git-push`** — shows the current branch, its upstream (or the lack of one), and
  ...
  via `just push`. Never force-pushes — that's explicitly out of scope for this
